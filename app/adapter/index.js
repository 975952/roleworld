...
(function (global) {
  ...
  const DEFAULT_LOCAL_SETTINGS = Object.freeze({
    ...
    // 且已发过的内容下次按缓存命中计价，全带上既不丢内容也不贵。
    // 只有用户自己填了正数才按上限从最近往前截断。
    history_token_budget: 0,
    // 本地 / 自定义端点的上下文上限（token）。只用于"这一轮会不会超上下文"的估算：
    // 各家本地模型差别很大（8k ~ 128k），按默认 32768 猜，猜错了用户能在这里改。
    // DeepSeek 官方不走这个值（按 1M 算）。
    local_context: 32768,
