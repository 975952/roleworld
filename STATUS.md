...
| 文件 | 改了什么 |
|---|---|
| `app/tokens.css` | 四套风格色板重写；每套补 `--focus-grad`；顶部光带改小改淡并统一取 `--focus`（金/樱/林各自成色） |
| `app/glass.css` | 金色段扩成"带颜色的风格"段（金/樱/林/羊皮纸共用同一批表面渐变规则，透明度减半）；新增羊皮纸纸纹层 |
...
> 深林这条我按"很淡的浅绿"理解成：**亮色**主题下底色就是浅绿；**暗色**主题下底色仍是近黑、
> 绿色用很淡的浅绿（不然白字读不出来）。如果你要的是暗色下也整片浅绿底，说一句就翻过来。
