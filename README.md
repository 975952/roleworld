...
**这个仓库本身不含任何角色数据。** 角色、记忆书、示例对话都以「内容包」的形式放在
`packs/` 目录下，应用启动时自动安装缺失的部分（绝不覆盖你改过的内容）。

```
packs/index.json                     # 内容包清单
packs/<包名>/characters/<头像>.json   # CCv3 角色卡
packs/<包名>/worlds/<名字>.json       # 世界书 / 记忆书
```

想换题材（奇幻、校园、原创世界观），只要换一个包。想删掉某个包，直接删目录即可。
...
已完成：本地适配层、去掉账号系统、三个页面接入适配层、内容包机制、存档导出导入、回归测试。
