...
| `local-app-check.cjs` | 端到端：起静态服务器 + 假模型端点，用无头 Chrome 打开三个页面，验证启动、流式回复、记忆书、模型配置面板、账号入口不可见；含「内容包自动安装」与「停用内容包后的空库启动」两组用例 | `node tests/local-app-check.cjs` |
| `desktop-smoke.cjs` | 启动真正编译出来的 `roleworld.exe`，验证数据确实以普通文件落盘（6 角色 / 4 记忆书 / 6 头像）。需要先 `pnpm desktop:build` | `node tests/desktop-smoke.cjs` |
